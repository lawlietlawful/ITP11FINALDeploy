...
<head>
    ...
    <title>Application Received | Secure Tracking Issued</title>
    <meta name="description" content="Your barangay document request has been submitted successfully. Save your secure tracking code to monitor real-time verification status.">
...
<body class="bg-[#f8fafc] text-slate-800 antialiased selection:bg-brand-500 selection:text-white min-h-screen flex flex-col">
    ...
    <main class="flex-1 max-w-xl w-full mx-auto px-6 py-10 flex flex-col justify-center">

        {{-- Top Success Status Animated Badge --}}
        <div class="text-center mb-6 animate-[scaleUp_0.4s_ease-out]">
            <div class="w-20 h-20 bg-emerald-50 rounded-full border-8 border-emerald-500/10 flex items-center justify-center mx-auto mb-4 text-emerald-500 shadow-inner">
                <svg class="w-10 h-10 animate-[bounce_1s_ease-in-out_2.5]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
            </div>
            <span class="text-xs font-black text-emerald-600 uppercase tracking-widest block">Application Authenticated</span>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight mt-1">Request Successfully Lodged</h1>
        </div>

        {{-- Ticket Design Card --}}
        <div class="ticket-card px-8 py-8 mb-6 overflow-hidden">
            {{-- Ticket Header Section --}}
            <div class="text-center">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest block mb-2">Secure Tracking Identifier</span>
                
                {{-- Tracking code display with Alpine interactive copy --}}
                <div x-data="{ copied: false }" class="relative max-w-sm mx-auto">
                    <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-3 group">
                        <span class="font-mono text-xl sm:text-2xl font-extrabold tracking-widest text-slate-900 block truncate select-all">{{ $docRequest->tracking_code }}</span>
                        
                        <button @click="navigator.clipboard.writeText('{{ $docRequest->tracking_code }}'); copied = true; setTimeout(() => copied = false, 2500)" 
                            ...
                            <svg x-show="copied" class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                            
                            {{-- Tooltip feedback --}}
                            <span x-show="copied" x-transition class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 rounded bg-slate-900 text-white text-[10px] font-bold whitespace-nowrap shadow-md">Copied Identifier!</span>
                        </button>
                    </div>
                </div>

                <p class="text-slate-400 text-xs mt-3 max-w-xs mx-auto leading-relaxed">
                    Please retain this exact token. It acts as your primary look-up credential to query processing timeline progress.
                </p>
            </div>
            ...
            <div class="space-y-3.5 text-xs">
                ...
                <div class="flex justify-between items-start gap-4">
                    ...
                    <span class="font-bold text-slate-800 text-right">{{ $docRequest->documentType->processing_days }} {{ Str::plural('day', $docRequest->documentType->processing_days) }}</span>
                </div>
                ...
                <div class="flex justify-between items-start gap-4 pt-3 border-t border-slate-100">
                    ...
                    <span class="font-extrabold text-slate-900 text-right text-sm">₱{{ number_format($docRequest->documentType->fee, 2) }}</span>
                </div>
            </div>

            {{-- Step checklist footer inside receipt --}}
            <div class="mt-6 pt-4 border-t border-slate-100 bg-slate-50 -mx-8 -mb-8 p-6 text-center">
                <span class="text-[10px] font-extrabold text-slate-400 block uppercase tracking-wider mb-2">Next Verification Lifecycle</span>
                <div class="flex items-center justify-center gap-2 text-xs font-medium text-slate-600">
                    <span class="text-amber-500 font-bold">1. Staff Verification</span>
        ...
        {{-- Direct Action Calls --}}
        <div class="space-y-3">
            <a href="{{ route('public.track', ['tracking_code' => $docRequest->tracking_code]) }}" class="w-full py-4 rounded-xl font-bold text-xs text-white bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 shadow-xl shadow-brand-500/20 hover:shadow-brand-500/30 transition-all text-center uppercase tracking-wider block">
                Monitor Live Status Timeline
            </a>
            
            <div class="grid grid-cols-2 gap-3">
                <a href="{{ route('public.request') }}" class="py-3 rounded-xl font-bold text-xs text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-colors text-center block">
                    Submit Another
                </a>
                <a href="{{ route('public.home') }}" class="py-3 rounded-xl font-bold text-xs text-slate-500 hover:text-slate-800 transition-colors text-center block">
                    Return to Home
                </a>
            </div>
        </div>
    ...
    <footer class="bg-slate-900 text-slate-500 py-6 border-t border-slate-800 mt-12 text-xs text-center">
        <div class="max-w-4xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p>&copy; {{ date('Y') }} VistáBarangay. Encrypted Verification.</p>
